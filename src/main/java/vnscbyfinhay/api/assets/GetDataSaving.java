...
public class GetDataSaving {
    ...
    public GetDataSaving(String env) {
        if ("dev".equals(env)) {
    ...
    public JsonPath getAPIDataSaving(Integer uid) throws Exception {
        return given().header( "Authorization","Bearer "+ GetToken.getAPIToken(uid))
                .when()
                .get(configPath.GET_DATA_SAVING +uid + "/saving-books/summary" )
                .then()
                .statusCode(200)
