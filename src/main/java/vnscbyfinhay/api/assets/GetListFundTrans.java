...
public class GetListFundTrans {
    ...
    public GetListFundTrans(String env) {
        if ("dev".equals(env)) {
    ...
    public JsonPath getAPIListFundTrans(Integer uid) throws Exception {
        return given().header( "Authorization","Bearer "+ GetToken.getAPIToken(uid))
                .when()
                .get(configPath.GET_LIST_FUND_TRANS +uid + "/orders" )
                .then()
                .statusCode(200)
